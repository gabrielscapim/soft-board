...
export function getLineGuideStops (params: GetLineGuideStopsParams): GetLineGuideStopsResult {
  const vertical: number[] = []
  const horizontal: number[] = []

  for (const component of params.flexComponents) {
    if (component.id === params.selectedFlexComponentId) continue
